...
export default function DeliveringOrder({orders}:{orders: Order[]}) {
    const deliveringOrders = orders.filter(order => order.orderStatus === 'pending').reverse()
    if (!deliveringOrders || deliveringOrders.length <= 0) return (<p>No delivering orders</p>)

    return (
        <div className="w-full">
            <h2 className="font-bold text-xl mb-4 mt-6">Pending Orders</h2>
    ...
    return (
        <div className="space-y-4">
            ...
            {orderItems.map((product: any) => (
                <ProductItem item={product} key={product.id} quantity={product.quantity}/>
            ))}
        </div>
    )
}

const ProductItem = ({item, quantity}: {item: any, quantity: any}) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col justify-between">
                <p className="font-semibold text-lg">{data?.name} x {quantity}</p>
